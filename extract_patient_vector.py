...
logging.basicConfig(filename=parameters.log_file_name, level=logging.INFO)

train_csv = pd.read_csv(parameters.obesity_train_label)

train_patient_name = list(train_csv['patient_id'])
n_patient = len(train_patient_name)
def generate_token_embedding(subject_id):
    ...
    patient_file = parameters.obesity_train_file_folder + str(subject_id) + '.txt'
...
input_x = tf.placeholder(tf.int32,
                         [None, parameters.max_document_length, parameters.max_sentence_length],name="input_x")
...
num_batch = int(math.ceil(n_patient / parameters.n_batch))
...
with tf.Session() as sess:
    ...
    for i in tqdm(range(num_batch)):
        ...
        for j in range(len(tmp_train_patient_name)):
            np.save(parameters.obesity_vector_directory + tmp_train_patient_name[j] + ".npy", tmp_patient_vector[j])
